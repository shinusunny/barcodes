...
class Barcode {
  constructor(ean) {
    ...
    this.twoMinus = [0, 2, 4, 6, 8, 9, 1, 3, 5, 7];
    this.threePlus = [0, 3, 6, 9, 2, 5, 8, 1, 4, 7];
    this.fivePlus = [0, 5, 1, 6, 2, 7, 3, 8, 4, 9];
    this.fiveMinus = [0, 5, 9, 4, 8, 3, 7, 2, 6, 1];
    this.threeAdd = [0, 7, 4, 1, 8, 5, 2, 9, 6, 3];
    this.fiveAdd = [0, 9, 7, 5, 3, 1, 8, 6, 4, 2];
    ...
    this.gs1 = {
      '01': {
        ...
        fun(gtin) {
          console.log('01', gtin, this);
          this.gs1Dtls.gtin14 = gtin;
          this.gs1Dtls.ean13 = gtin.slice(1, 14);
        },
      },
      15: {
        len: 6,
        ...
        fun(expiry) {
          this.gs1Dtls.expiry = expiry;
          this.gs1Dtls.expiryType = '15';
        },
      },
      16: {
        len: 6,
      ...
      3920: {
        ...
        fun(ai, price) {
          this.gs1Dtls.priceMinor = this.price392(ai, price);
        },
      },
      3921: {
        len: 7,
  ...
  cdValidate() {
    ...
    if (isNaN(this.ean) || this.ean <= 0 || !this.types[this.ean.length + 1]) {
      const errMsg = Object.entries(this.types)
        .map(([k, v]) => `${k - 1} (${v})`)
        .join(', ');
      alert(`Entered length: ${this.ean.length}\nValid lengths: ${errMsg}`);
      valid = false;
    }
    return valid;
  }
  ...
  pcdValidate() {
    ...
    if (
      ...
      this.ean.length !== 13 ||
      +this.ean[0] !== 2 ||
      ![1, 2, 3, 4, 7, 8, 9].includes(+this.ean[1])
    ) {
      const errMsg = Object.entries(this.types)
  ...
  gs1Validate() {
    this.gs1.Barcode = this.ean;
    const eanArr = [...this.ean];
    let eanRem = this.ean;
    let ai = '';
    let valid = false;
    let re, found;
      ...
      if (ai in this.gs1) {
        ...
        if (gs1Item.FNC1) {
          ...
          if (found) {
            ...
            gs1Item.fun.call(this, ai, found[1]);
          }
        } else {
          ...
          gs1Item.fun.call(this, found[1]);
        }
        ai = '';
      }
    }
    console.log(this.gs1.Barcode);
    return valid;
  }

  checkDigit() {
    const digits = [...this.ean].reverse();
...
cdBtn.addEventListener('click', function (e) {
  ...
  const barcode = new Barcode(cdInp.value);
  if (barcode.cdValidate()) {
    const cd = barcode.checkDigit();
    cdEan.textContent = '';
    cdEan.insertAdjacentHTML('beforeend', `${barcode.ean}<span class="cd-cd">${cd}</span>`);
    cdTyp.textContent = `(${barcode.types[barcode.ean.length + 1]})`;
...
pdBtn.addEventListener('click', function (e) {
  ...
  let pde = '';
  const barcode = new Barcode(pdInp.value);
  const pdi = barcode.ean;
  if (barcode.pcdValidate()) {
    let ean = `${pdi.slice(0, pdi.length - 1)}`.padStart(12, '0');
    const prc4 = [3, 4].includes(+ean[1]);
    let pdp = prc4 ? 7 : 6;
    let pd = prc4 ? barcode.pcdFourDigitPrice() : barcode.pcdFiveDigitPrice();
    pde = `${ean.slice(0, pdp)}${pd}${ean.slice(pdp + 1)}`;
    barcode.ean = pde;
    pdEan.textContent = pde.concat(barcode.checkDigit());
